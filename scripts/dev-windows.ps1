...
if (-not $SkipDatabase) {
  ...
  # Canonical baseline files in order. The rest of database/*.sql are
  # feature modules that can be applied in any order.
  $baseline = @(
    'database/schema.sql',
  ...
  # Apply every other database/*.sql in alphabetical order. Each file is
  # idempotent; re-running is safe.
  $rest = Get-ChildItem -Path (Join-Path $RepoRoot 'database') -Filter *.sql |
          Where-Object { $_.Name -notin @('schema.sql','auth-hardening.sql','admin-user-management.sql','feature-flags.sql','seed-default-admin.sql','accounts-teardown.sql') } |
          Sort-Object Name
  foreach ($f in $rest) {
