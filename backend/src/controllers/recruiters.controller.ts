...
export const list: RequestHandler = async (_req, res) => {
  ...
  if (error && /recruiter_managers/i.test(error.message)) {
    const fallback = await supabaseAdmin
      .from('recruiters')
      .select('*, user:users!user_id(id, email, full_name), manager:users!manager_id(id, email, full_name)')
      .order('created_at', { ascending: false });
...
export const get: RequestHandler = async (req, res) => {
  ...
  if (error && /recruiter_managers/i.test(error.message)) {
    const fb = await supabaseAdmin
      .from('recruiters')
      .select('*, user:users!user_id(id, email, full_name, phone, avatar_url), manager:users!manager_id(id, email, full_name)')
...
export const onboard: RequestHandler = async (req, res) => {
  ...
  if (Object.keys(userPatch).length > 0) {
    const { error: uErr } = await supabaseAdmin
      .from('users').update(userPatch).eq('id', req.user.id);
  ...
  const { data, error } = await supabaseAdmin
    .from('recruiters')
    .upsert(
      { user_id: req.user.id, ...recruiterRow },
      { onConflict: 'user_id' }
    )
    .select()
    .single();
...
function junctionMissing(message: string | undefined): boolean {
  ...
  return /recruiter_managers/i.test(message) &&
...
export const addManager: RequestHandler = async (req, res) => {
  ...
  const { data: rec } = await supabaseAdmin
    .from('recruiters').select('id').eq('id', recruiterId).maybeSingle();
  if (!rec) throw httpError(404, 'Recruiter not found');

  const { data: target } = await supabaseAdmin
    .from('users').select('id, role').eq('id', parsed.data.manager_id).maybeSingle();
  if (!target) throw httpError(404, 'Manager user not found');

  // Probe whether the junction table exists.
  const { data: existing, error: existErr } = await supabaseAdmin
    .from('recruiter_managers').select('manager_id, is_primary').eq('recruiter_id', recruiterId);
  ...
  if (junctionMissing(existErr?.message)) {
    ...
    const { data, error } = await supabaseAdmin
      ...
      .eq('id', recruiterId).select().single();
    if (error) throw httpError(500, error.message);
    res.status(201).json({
      ...data,
      _degraded: 'recruiter_managers table not migrated — one supervisor at a time. Run database/recruiter-managers.sql for multi-manager support.',
    });
    return;
  }
  ...
  if (wantPrimary) {
    await supabaseAdmin
      .from('recruiter_managers').update({ is_primary: false }).eq('recruiter_id', recruiterId);
  }
  ...
  if (wantPrimary) {
    await supabaseAdmin.from('recruiters')
      .update({ manager_id: parsed.data.manager_id }).eq('id', recruiterId);
  }
...
export const removeManager: RequestHandler = async (req, res) => {
  ...
  const { data: removed, error: lookupErr } = await supabaseAdmin
    .from('recruiter_managers')
    .select('is_primary')
    .eq('recruiter_id', recruiterId).eq('manager_id', managerId).maybeSingle();

  if (junctionMissing(lookupErr?.message)) {
    // Pre-migration fallback — clear recruiters.manager_id only if it matches.
    await supabaseAdmin
      .from('recruiters').update({ manager_id: null })
      .eq('id', recruiterId).eq('manager_id', managerId);
  ...
  const { error } = await supabaseAdmin
    .from('recruiter_managers')
    .delete()
    .eq('recruiter_id', recruiterId).eq('manager_id', managerId);
  if (error) throw httpError(500, error.message);

  if (removed?.is_primary) {
    const { data: next } = await supabaseAdmin
      .from('recruiter_managers').select('manager_id')
      .eq('recruiter_id', recruiterId).order('assigned_at').limit(1).maybeSingle();
    if (next?.manager_id) {
      await supabaseAdmin
        .from('recruiter_managers').update({ is_primary: true })
        .eq('recruiter_id', recruiterId).eq('manager_id', next.manager_id);
      await supabaseAdmin
        .from('recruiters').update({ manager_id: next.manager_id }).eq('id', recruiterId);
    } else {
      await supabaseAdmin
        .from('recruiters').update({ manager_id: null }).eq('id', recruiterId);
    }
  }
  res.json({ ok: true });
};
...
export const setPrimaryManager: RequestHandler = async (req, res) => {
  ...
  if (junctionMissing(lookupErr?.message)) {
    ...
    const { error } = await supabaseAdmin
      .from('recruiters').update({ manager_id: managerId }).eq('id', recruiterId);
    if (error) throw httpError(500, error.message);
    res.json({ ok: true, _degraded: true });
    return;
  }
