...
export const ats: RequestHandler = async (req, res) => {
  ...
  if (!resume_text || !job_description) throw httpError(400, 'resume_text and job_description required');
  res.json(await atsScore(resume_text, job_description));
};

export const vendorEmail: RequestHandler = async (req, res) => {
  const { consultantName, consultantSkills, consultantExperienceYears, jobTitle, jobDescription, vendorName, recruiterName } = req.body ?? {};
