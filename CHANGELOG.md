...
All notable changes to this project. Entries are grouped by release version and use the section headings from [Keep a Changelog](https://keepachangelog.com/en/1.1.0/) — **Added**, **Changed**, **Fixed**, **Removed**, **Security**, **Deprecated**.

Releases are cut via `bash scripts/release.sh <patch|minor|major>` and pushed via `git push --tags`. The [Release workflow](.github/workflows/release.yml) picks up the tag and publishes a GitHub Release with the matching section copied as the release body.
...
- `frontend/src/utils/fileUrl.ts` — central resolver/opener for backend-served URLs. Handles absolute storage URLs vs relative API paths; applies `noopener,noreferrer`.
...
- `database/seed-default-admin.sql` — standalone-safe SQL seed for the SUPER_ADMIN account (credentials now sourced from env vars, no defaults in source).
...
- **Local `signInWithPassword`** — backend-owned (bcrypt). Used by the change-password flow to re-verify the current password before rotation.
...
- **`Path A` in AcceptInvitation** — the legacy `auth.updateUser({password})` branch could overwrite the signed-in user's password if they clicked an invite link.
...
- **Custom forgot-password / reset-password flow** (no third-party recovery emails).
...
- **HireOrbit AI → HireOrbit AI** across UI, emails, and metadata.

---

## [0.1.0] — Initial

Original HireOrbit AI release. Documented in the legacy README; superseded.
