  ...
  for (const role of ROLES) {
    test(`${role} sees exactly its matrix labels`, async ({ page }) => {
      ...
      for (const label of expected) {
        ...
        // item's height, which would flake a strict visibility check. The
        // Sidebar gates the nav on `role` being resolved, so the item set is
        // stable (no all-items→filtered churn) by the time 'Dashboard' is
        // visible above; a 12s ceiling covers slow CI render.
