...
export function Header({ title, crumbs, onMenuClick: _onMenuClick }: Props) {
  ...
  // A Back button on every page except the dashboard home (where "back" would
  // leave the app). Uses browser history so it returns to wherever the user came
  // from; falls back to the dashboard when there's no in-app history.
  const isHome = loc.pathname === '/dashboard' || loc.pathname === '/';
  const goBack = () => {
  ...
  return (
    ...
    >
      {/* ── Back button (every page except the dashboard home) ── */}
      {!isHome && (
        <button
          type="button"
          aria-label="Go back"
          onClick={goBack}
          className="w-8 h-8 -ml-1 rounded-[9px] grid place-items-center transition-colors hover:bg-hover shrink-0 press"
