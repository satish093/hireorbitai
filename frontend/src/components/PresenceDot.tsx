...
export function PresenceDot({
  lastSeenAt, size = 10, className, title,
}: {
  ...
  return (
    <span
      className={clsx('relative inline-block rounded-full ring-2 ring-white', COLOR[status], className)}
    ...
    >
      ...
      {status === 'online' && (
        <span className={clsx('absolute inset-0 rounded-full opacity-60 animate-ping', COLOR[status])} />
      )}
    </span>
  );
}
...
export function PresencePill({ lastSeenAt }: { lastSeenAt: string | null | undefined }) {
  const status = presenceStatusFromLastSeen(lastSeenAt);
  const tone =
    status === 'online' ? 'text-emerald-700 bg-emerald-50 border-emerald-100'
    : status === 'idle' ? 'text-amber-700 bg-amber-50 border-amber-100'
    : 'text-slate-500 bg-slate-50 border-slate-200';
  return (
    <span className={clsx('inline-flex items-center gap-1 text-[10px] font-semibold uppercase tracking-wider px-1.5 py-0.5 rounded-full border', tone)}>
      <span className={clsx('w-1.5 h-1.5 rounded-full', COLOR[status], status === 'online' && 'animate-pulse-soft')} />
