...
export function Reports() {
  ...
  return (
    <Layout title="Reports">
      ...
      {/* Tab strip */}
      <div className="flex items-end gap-5 border-b border-slate-200 mb-5 overflow-x-auto">
        {TABS.map((t) => (
          <button
            key={t.key}
          ...
          >
            ...
            <span
              aria-hidden="true"
              className={clsx(
                'absolute left-0 right-0 -bottom-px h-0.5 bg-slate-900 rounded-full origin-center transition-transform duration-200 ease-out',
    ...
    <div>
      ...
      <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
            {rows.map((r) => (
              <tr key={r.recruiter_id} className="border-t border-slate-100 hover:bg-slate-50">
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
      <table className="w-full text-sm">
        ...
        <tbody>
          {rows.map((c) => (
            <tr key={c.consultant_id} className="border-t border-slate-100 hover:bg-slate-50">
              <td className="px-3 py-2.5">
                <div className="inline-flex items-center gap-2">
...
function PlacementAnalytics({ data }: { data: PlacementSnapshot | null }) {
  if (data === null) return <p className="text-sm text-slate-500">Loading…</p>;
  const f = data.funnel;
  const steps = [
    { label: 'Submissions', value: f.submissions, color: 'bg-slate-400' },
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-5">
        <h3 className="font-semibold text-slate-900 mb-3">Placement funnel</h3>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-5">
          <h3 className="font-semibold text-slate-900 mb-3">Top vendors by submissions</h3>
          {data.top_vendors.length === 0 ? (
            <p className="text-xs text-slate-400 italic">No vendor activity yet.</p>
          ) : (
            <div className="space-y-2">
              {data.top_vendors.map((v) => (
                <div key={v.vendor_id} className="flex items-center justify-between text-sm">
                  <span className="text-slate-700">{v.name}</span>
                  <span className="tabular-nums font-semibold text-slate-900">{v.submissions}</span>
                </div>
              ))}
            </div>
          )}
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-5">
        <h3 className="font-semibold text-slate-900 mb-3">Applications by status</h3>
        <div className="grid grid-cols-2 md:grid-cols-6 gap-2">
          {Object.entries(data.applications_by_status).map(([k, v]) => (
            <div key={k} className="border border-slate-200 rounded-lg p-2 text-sm">
              <div className="text-slate-500 text-[10px] uppercase tracking-wide">{k}</div>
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          <thead className="bg-slate-50 border-b border-slate-200">
            <tr className="text-[10px] uppercase tracking-widest text-slate-500 font-semibold">
