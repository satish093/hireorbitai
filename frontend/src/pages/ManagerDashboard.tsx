...
import {
  TASK_STATUSES, TASK_STATUS_LABEL, TaskStatus, TASK_PRIORITIES, TaskPriority,
} from '../types';
...
export function ManagerDashboard() {
  ...
  useEffect(() => {
    api.get('/reports/manager-summary')
      .then((r) => setSummary(r.data))
      .catch((e) => pushError('summary', e?.response?.data?.error ?? 'reports/manager-summary failed'));
    api.get('/tasks/metrics')
      .then((r) => setTasks(r.data))
      .catch((e) => pushError('tasks', e?.response?.data?.error ?? 'tasks/metrics failed — run database/tasks.sql'));
    api.get('/jobs')
  ...
  return (
    <Layout title="Manager dashboard">
      ...
      <div className="mb-8 flex flex-wrap items-end justify-between gap-4">
        <div>
          <div className="text-[11px] font-semibold tracking-widest text-slate-500 mb-2">{dateStr}</div>
          ...
          {tasks ? (
            <p className="text-sm text-slate-600 mt-2">
              ...
              <span className={tasks.overdue > 0 ? 'font-semibold text-red-600' : 'font-semibold text-slate-900'}>{tasks.overdue} overdue</span>
              , and{' '}
              <span className={tasks.due_today > 0 ? 'font-semibold text-amber-700' : 'font-semibold text-slate-900'}>{tasks.due_today} due today</span>.
            </p>
          ) : (
        ...
        <div className="flex items-center gap-2">
          <Link to="/jobs" className="bg-white border border-slate-200 rounded-lg px-3 py-1.5 text-sm hover:bg-slate-50 inline-flex items-center gap-1.5">
      ...
      {(profile?.role === 'SUPER_ADMIN' || profile?.role === 'MANAGER') &&
        ...
        <QuickActions hasJobs={!!jobsCount && jobsCount > 0} hasTasks={!!tasks && tasks.total > 0} />
      )}
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6 stagger-children">
        <DashboardCard
          ...
          value={tasks?.open ?? '—'}
          accent="slate"
          hint={tasks ? <span className="text-slate-500">{tasks.total} total</span> : <span className="text-slate-400">Run tasks migration</span>}
        />
        <DashboardCard
          label="Overdue"
          value={<span className={tasks && tasks.overdue > 0 ? 'text-red-600' : ''}>{tasks?.overdue ?? '—'}</span>}
          accent="red"
          hint={tasks && tasks.critical_open > 0 ? <span className="text-red-600">{tasks.critical_open} critical</span> : null}
        />
        <DashboardCard
          label="Due this week"
          value={tasks?.due_this_week ?? '—'}
          accent="amber"
          hint={summary ? <span className="text-slate-500">across {summary.recruiters_count} recruiters</span> : null}
        />
        <DashboardCard
          label="Completed (7d)"
              ...
              {TASK_STATUSES.map((s) => (
                <div key={s} className="text-sm">
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-[11px] text-slate-500 uppercase tracking-wide">{TASK_STATUS_LABEL[s]}</span>
                  </div>
                  <div className="text-lg font-semibold mt-0.5 tabular-nums text-slate-900">{tasks.by_status[s] ?? 0}</div>
                </div>
              ))}
            </div>
          </div>
                  ...
                  <div key={p}>
                    <div className="flex items-center justify-between text-sm mb-1">
                      <span className="inline-flex items-center gap-1.5">
                        ...
                        <span className="text-slate-700">{p.charAt(0) + p.slice(1).toLowerCase()}</span>
                      </span>
      ...
      {summary && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6 stagger-children">
          <DashboardCard label="Active consultants" value={summary.consultants_by_status.ACTIVE ?? 0} accent="green" />
          <DashboardCard label="Paused" value={summary.consultants_by_status.PAUSED ?? 0} accent="amber" />
          <DashboardCard label="Placed" value={summary.consultants_by_status.PLACED ?? 0} accent="blue" />
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-2">
              {Object.entries(summary.applications_by_status).map(([k, v]) => (
                <div key={k} className="border border-slate-200 rounded-lg px-3 py-2.5">
                  <div className="text-slate-500 text-[10px] uppercase tracking-widest font-semibold">{k.replace(/_/g, ' ')}</div>
                  <div className="text-2xl font-semibold tabular-nums text-slate-900 mt-0.5">{v}</div>
                </div>
              ))}
            </div>
          )}
...
function QuickActions({ hasJobs, hasTasks }: { hasJobs: boolean; hasTasks: boolean }) {
  return (
    <div className="mb-6 bg-gradient-to-br from-brand-50 to-violet-50 border border-brand-100 rounded-xl p-5">
      <div className="text-[10px] font-semibold tracking-widest text-brand-700 uppercase mb-1">First steps</div>
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between">
        ...
        {done && <span className="text-[10px] text-emerald-700 bg-emerald-50 border border-emerald-100 px-1.5 py-0.5 rounded">Done</span>}
      </div>
  ...
  return (
    <div className="h-2 rounded-full bg-slate-100 overflow-hidden flex">
      {TASK_STATUSES.map((s) => {
        ...
        return <div key={s} className={STATUS_BAR_COLOR[s]} style={{ width: `${pct}%` }} title={`${TASK_STATUS_LABEL[s]}: ${n}`} />;
      })}
    </div>
  );
}
