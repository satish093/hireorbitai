...
export function AdminUsers() {
  ...
  const queryString = useMemo(() => {
    const p: Record<string, string> = { page: String(page), page_size: String(page_size), sort, order };
    if (q) p.q = q;
    if (role) p.role = role;
  ...
  useEffect(() => {
    ...
    setLoading(true);
    api.get<ListResponse>('/admin/users', { params: queryString, signal: ctl.signal })
    ...
    try {
      ...
      setData({
        ...
        rows: data.rows.map((r) =>
          r.id === u.id
            ? { ...r, status: (updated.status ?? status) as Status, status_reason: updated.status_reason ?? null }
            : r,
        ),
      });
      toast.success(
  ...
  return (
    <Layout title="Admin · Users">
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-3 mb-4 flex flex-wrap items-end gap-3">
        ...
        {(q || role || status) && (
          <Button variant="ghost" onClick={() => setParams(new URLSearchParams(), { replace: false })}>
            Reset
          </Button>
        )}
      </div>
        ...
        columns={[
          {
            ...
            render: (u: AdminUserRow) => (
              <Link to={`/admin/users/${u.id}`} className="inline-flex items-center gap-2 hover:bg-slate-50 rounded -mx-1 px-1 py-0.5">
                <Avatar name={u.full_name} email={u.email} size={28} />
                <div className="leading-tight">
                  <div className="text-sm font-medium text-slate-900 hover:underline">{u.full_name ?? u.email}</div>
          ...
          },
          {
            ...
            render: (u: AdminUserRow) =>
              u.last_login_at
                ? <span className="text-xs text-slate-700" title={new Date(u.last_login_at).toLocaleString()}>{relative(u.last_login_at)}</span>
          ...
          },
          {
            ...
            render: (u: AdminUserRow) => (
              <RowActions
                ...
                isSelf={me?.id === u.id}
                busy={busyRowId === u.id}
                onView={() => { /* navigation handled by Link inside menu */ }}
      ...
      {!loading && totalPages > 1 && (
        <div className="flex items-center justify-between mt-4 text-sm">
          <div className="text-slate-500">
            Page {page} of {totalPages} · showing {data?.rows.length ?? 0} of {total.toLocaleString()}
          </div>
...
function RowActions({ row, isSelf, busy, onSendReset, onDeactivate, onSuspend, onReactivate }: RowActionsProps) {
