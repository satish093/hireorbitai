...
import {
  PriorityBadge, TaskStatusBadge, DuePill, AssigneeChip, Avatar, TagPill, shortId, isOverdue,
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      for (const r of recs.data ?? []) {
        if (r.user_id) seen.set(r.user_id, { id: r.user_id, full_name: r.user?.full_name, email: r.user?.email, role: 'RECRUITER' });
      }
      for (const c of cons.data ?? []) {
        if (c.user_id) seen.set(c.user_id, { id: c.user_id, full_name: c.user?.full_name, email: c.user?.email, role: 'CONSULTANT' });
      }
      if (profile) seen.set(profile.id, { id: profile.id, full_name: profile.full_name, email: profile.email, role: profile.role });
    ...
    >
      ...
      <div className="flex items-center gap-3 mb-5">
        ...
        <div className="ml-3 inline-flex rounded-lg bg-slate-100 p-0.5 text-sm">
          <ViewToggleBtn active={view === 'board'} onClick={() => setParams({ view: 'board' })}>Board</ViewToggleBtn>
          <ViewToggleBtn active={view === 'list'} onClick={() => setParams({ view: 'list' })}>List</ViewToggleBtn>
        </div>

        <div className="ml-auto flex items-center gap-2">
          <Link to="/tasks/me" className="text-sm text-slate-600 hover:text-slate-900 inline-flex items-center gap-1">
            Assigned to me →
          </Link>
          {isManager && (
            <button
...
function ViewToggleBtn({ active, onClick, children }: { active: boolean; onClick: () => void; children: ReactNode }) {
  return (
    <button
      onClick={onClick}
      className={clsx(
        'px-3 py-1 rounded-md transition',
        active ? 'bg-white text-slate-900 shadow-sm font-medium' : 'text-slate-600 hover:text-slate-900',
      )}
    >
      {children}
    </button>
...
// ---------------------------------------------------------------------------
function FilterBar({
  filters, setFilters, users, isManager,
}: {
  filters: FilterState; setFilters: (f: FilterState) => void;
  users: { id: string; full_name?: string; email: string; role: string }[];
  isManager: boolean;
}) {
  const pill = 'border border-slate-200 bg-white rounded-full pl-3 pr-2.5 py-1 text-sm text-slate-700 inline-flex items-center gap-1.5 hover:bg-slate-50 focus-within:ring-2 focus-within:ring-brand-500/40';
  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      <div className={pill}>
        ...
        <input
          type="text" placeholder="Search title…" value={filters.q ?? ''}
      ...
      <PillSelect
        ...
        options={[{ value: '', label: 'Open' }, ...TASK_STATUSES.map(s => ({ value: s, label: TASK_STATUS_LABEL[s] }))]}
      />
      <PillSelect
        label="Priority"
        value={filters.priority ?? ''}
        onChange={(v) => setFilters({ ...filters, priority: (v || '') as TaskPriority | '' })}
        options={[{ value: '', label: 'All' }, ...TASK_PRIORITIES.map(p => ({ value: p, label: p }))]}
      />
      {isManager && (
        <PillSelect
          label="Assignee"
          ...
          options={[{ value: '', label: 'All' }, ...users.map(u => ({ value: u.id, label: `${u.full_name ?? u.email}` }))]}
        />
      )}
      <button
...
// Board view
// ---------------------------------------------------------------------------
function BoardView({ tasks, onStatusChange }: { tasks: Task[]; onStatusChange: (id: string, s: TaskStatus) => void }) {
        ...
        >
          <div className="flex items-center justify-between mb-2 px-1">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs font-medium text-slate-500 tabular-nums bg-slate-100 rounded px-1.5">{grouped[status].length}</span>
            </div>
          </div>
          <div className="space-y-2">
            {grouped[status].map(t => <BoardCard key={t.id} task={t} onDragStart={onDragStart} />)}
...
function BoardCard({ task, onDragStart }: { task: Task; onDragStart: (e: DragEvent<HTMLElement>, id: string) => void }) {
  return (
    <Link
      to={`/tasks/${task.id}`}
      draggable
    ...
    >
      ...
      {task.consultant && (
        <div className="mt-2 inline-flex items-center gap-1.5 text-xs text-slate-600 bg-slate-50 border border-slate-100 rounded-full pl-1 pr-2 py-0.5">
          <Avatar name={task.consultant.user?.full_name} email={task.consultant.user?.email} size={18} />
    ...
    <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
      <table className="w-full text-sm">
        ...
        <tbody>
          {tasks.length === 0 && (
            <tr>
              <td colSpan={7} className="px-3 py-8 text-center text-slate-400 italic">No tasks match these filters.</td>
            </tr>
          )}
          {tasks.map((t) => (
            <tr
              key={t.id}
              className={clsx('border-t border-slate-100 hover:bg-slate-50', isOverdue(t) && 'bg-red-50/30')}
            >
              <td className="px-3 py-2 text-[11px] font-mono text-slate-500">{shortId(t.id)}</td>
              <td className="px-3 py-2">
                <Link to={`/tasks/${t.id}`} className="text-slate-900 hover:underline">{t.title}</Link>
                {t.tags && t.tags.length > 0 && (
                  <div className="mt-0.5 flex flex-wrap gap-1">
                    {t.tags.slice(0, 3).map(tg => <TagPill key={tg} tag={tg} />)}
                  </div>
                )}
              </td>
              <td className="px-3 py-2"><TaskStatusBadge status={t.status} /></td>
              <td className="px-3 py-2"><PriorityBadge priority={t.priority} /></td>
              <td className="px-3 py-2"><AssigneeChip task={t} /></td>
              <td className="px-3 py-2">
                {t.consultant
                  ? <span className="inline-flex items-center gap-1.5 text-slate-700">
                      <Avatar name={t.consultant.user?.full_name} email={t.consultant.user?.email} size={20} />
                      <span className="truncate max-w-[110px]">{t.consultant.user?.full_name ?? t.consultant.user?.email}</span>
                    </span>
                  : <span className="text-slate-400 italic text-xs">—</span>}
              </td>
              <td className="px-3 py-2"><DuePill task={t} /></td>
            </tr>
          ))}
        </tbody>
      </table>
...
function CreateTaskModal({
  ...
  const [form, setForm] = useState<any>({ status: 'BACKLOG', priority: 'MEDIUM', tags: [] as string[] });
  ...
  function removeTag(t: string) { setForm({ ...form, tags: form.tags.filter((x: string) => x !== t) }); }
  ...
  return (
    <Modal
      open={true} onClose={onClose} title="Create task"
      footer={
    ...
    >
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          />
          <SelectInput label="Assignee" value={form.assignee_id ?? ''}
        ...
        {/* Tags */}
        <div>
          <span className="block text-xs font-semibold uppercase tracking-wide text-slate-500 mb-1.5">Tags</span>
          <div className="flex flex-wrap gap-1.5 items-center">
            {form.tags.map((t: string) => (
              <span key={t} className="inline-flex items-center gap-1 text-xs px-2 py-0.5 rounded-full bg-slate-100 text-slate-700">
                {t}
                <button onClick={() => removeTag(t)} className="text-slate-400 hover:text-slate-700">×</button>
              </span>
            ))}
            <input
              placeholder="+ Add tag"
