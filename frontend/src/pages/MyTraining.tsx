...
function PlanCard({ a, course }: { a: any; course: any | undefined }) {
  ...
  return (
    <Link
      ...
      className="block bg-surface border border-border rounded-2xl p-5 hover:border-brand-300 hover:shadow-sm transition"
    >
      <div className="flex items-center gap-2 flex-wrap mb-2">
        {category && <CourseCategoryBadge category={category} />}
        <TrainingStatusBadge status={a.status} />
        {typeof weekly_hours === 'number' && (
          <span className="text-[10px] text-muted">· ~{weekly_hours} h/week</span>
        )}
        {a.due_date && <span className="text-[10px] text-muted">· due {a.due_date}</span>}
      </div>

      <h2 className="text-lg font-semibold tracking-tight text-ink">{courseTitle}</h2>
      ...
      {objectives.length > 0 && (
        <div className="mt-2">
          ...
          <ul className="text-sm text-ink list-disc list-outside pl-5 space-y-0.5">
            ...
            {objectives.length > 3 && (
              <li className="italic text-muted">+{objectives.length - 3} more</li>
            )}
          </ul>
        </div>
      )}
