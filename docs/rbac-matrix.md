...
- **Invitations** — dropdown from `assignableRolesFor`; recruiter → CONSULTANT only; group restricted via `assertCanAssignGroup`; SUPER_ADMIN all roles. Non-admin callers (group leads + recruiters) get their invitee defaulted into their OWN group when no `group_id` is passed (the frontend hides the group picker for non-admins and surfaces "Invitee will be added to your group"). Recruiter-invited consultants therefore always land in the recruiter's group.
- **Consultants ↔ Recruiters invariant** — `consultant.user.group_id` must match the assigned `recruiter.user.group_id`. Group-lead reassigns enforce this by construction (both sides validated in-group). Admin-tier cross-group reassigns auto-sync `consultant.user.group_id` to the new recruiter's group on `assignRecruiter` so group-scoped surfaces stay coherent.
...
- **Reports** — MANAGER_TIER or DEVELOPER+reports; existing analytics tabs are global by design. **Submissions reports** (`/reports/submissions/by-consultant|by-recruiter`, Phase 13) are the exception: OPERATOR_TIER gate (recruiters included), but scoped per role — ADMIN/DEVELOPER+reports see all, group leads (HR/MANAGER) see only their group (fail-closed), RECRUITER sees only their own submissions, CONSULTANT denied at the gate. Source of truth is the `applications` table; date filters are timezone-safe (client passes local from/to, backend aggregates). Recruiters reach their own numbers via a self-service panel on the recruiter dashboard (the /reports page itself stays MANAGER_TIER).
...
- **Messages** — final policy (single source of truth = `permission.service.ts`, used by directory / send / thread fetch / read-receipt):
  - ADMIN_TIER (SUPER_ADMIN / CEO / CTO / DIRECTOR) → everyone.
  - HR_MANAGER / MANAGER (functionally one tier) → all managers + HR managers (org-wide peer chat) + admin tier + recruiters in own group + consultants in own group + recruiters they supervise via `recruiter_managers` (cross-group exception) and those recruiters' consultants. A lead with no `group_id` is fail-closed on the group branch (peers + admins still reachable).
  - RECRUITER → all other recruiters (org-wide peer chat) + admin tier + own manager / HR chain (assigned managers via junction + manager-tier in own group) + own assigned consultants. NEVER other recruiters' consultants.
  - CONSULTANT → assigned recruiter + that recruiter's manager chain (junction + in-group manager-tier). NEVER other consultants, NEVER unrelated recruiters.
  - DEVELOPER → no chat by default (capability grants are for admin surfaces, not messaging).
  - `canMessageUser` does a forward check only; the per-role rules are explicitly bidirectional, so a reverse fallback would only over-broaden. Strict assignment + group only — no reports_to chain, no prior-thread carry-over: a reassignment immediately makes the in-flight thread unreachable (fail-closed).
