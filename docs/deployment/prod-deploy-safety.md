...
| `1752000000000_auth-sessions-refresh-lookup` | `ADD COLUMN IF NOT EXISTS auth_sessions.refresh_lookup text` + partial index               | auth_sessions (additive)                             | none — new tokens populate it; existing sessions stay NULL until the next refresh                             |
